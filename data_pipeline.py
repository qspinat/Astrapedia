...
def process_hyg_stars(max_magnitude: float = Config.MAX_MAGNITUDE_LIMIT) -> Optional[Dict]:
...
def process_constellation_lines() -> Optional[Dict]:
                ...
                try:
                    ...
                    for i in range(len(hip_list) - 1):
                        ...
                        try:
                            hip1 = int(hip1_str)
                            hip2 = int(hip2_str)
                            constellations[current_constellation]['lines'].append([hip1, hip2])
                        except ValueError:
                            continue
...
def parse_ra_to_degrees(ra_str) -> Optional[float]:
...
def parse_dec_to_degrees(dec_str) -> Optional[float]:
...
def process_deep_sky_objects() -> Optional[Dict]:
    ...
    for _, row in df.iterrows():
        dso = {
            ...
            'common_names': row['Common names'] if pd.notna(row['Common names']) else None,
        }
        dso_list.append(dso)
    ...
    for obj_type, count in sorted(dso_by_type.items(), key=lambda x: x[1], reverse=True):
